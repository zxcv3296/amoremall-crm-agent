...
- **ML 페르소나 분류**: RandomForest + GradientBoosting 앙상블로 고객 페르소나 자동 분류
- **AI 발신 목적 추천**: Rule-based 엔진으로 최적 발신 목적 자동 추천
...
```
[고객 데이터] → [ML 페르소나 분류] → [AI 발신목적 추천] → [RAG 상품 검색] → [LLM 메시지 생성] → [결과 출력]
                (RF+GB 앙상블)        (Rule-based)         (FAISS)           (Claude/Qwen)
```
...
│
├── ml/                    # ML 모델
│   ├── persona_classifier.py   # 페르소나 분류기 (RF+GB 앙상블)
│   ├── purpose_recommender.py  # 발신목적 추천 (Rule-based)
│   └── persona_model.pkl       # 학습된 페르소나 분류 모델
│
...
| 페르소나 | 특징 |
|---------|------|
| **하이엔드 품격가** | 프리미엄 브랜드 충성, 베스트 큐레이션 선호 |
| **트렌디 Z세대** | 트렌드 민감, 신제품/프로모션 반응 높음 |
| **테크니컬 홈케어족** | 기술 기반 신제품 관심, 재구매율 높음 |
| **실속형 가계 수호자** | 할인/프로모션 민감, 재구매 주기 짧음 |
| **합리적 큐레이터** | 가성비 베스트 상품, 합리적 프로모션 선호 |
| **연구소 기반 해결사** | 피부 고민 해결용 신제품, 효능 검증 제품 선호 |
| **웰니스 힐링 탐험가** | 새로운 경험, 웰니스/시즌 제품 관심 |

### ML 모델 구조

- **앙상블 방식**: RandomForest + GradientBoosting (Soft Voting)
- **입력 특성**: 나이, 피부타입, 브랜드 충성도, 할인 민감도, 방문 빈도, 구매 금액 등 12개
...
- **Rule-based 엔진**: 페르소나 클러스터 + 이탈 위험도 기반 자동 추천
- **고객 변경 시 자동 실행**: API 키 불필요
